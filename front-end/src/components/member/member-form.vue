<template>
	<el-form ref="form" :model="form" label-width="80px" class="form-contain">
		...
		<el-form-item>
			<el-button type="primary" @click="onSubmit">保存会员</el-button>
...
export default {
  ...
  created() {
    let members_id = this.$route.query.member_id;

    if (members_id) {
      this.isNew = 0;

      this.func.ajaxPost(
        this.api.memberDetail,
        {
          members_id
        },
        res => {
          this.form = res.data.resultList;
          this.form.members_id = res.data.resultList.members_id;
        }
      );
    }
  }
